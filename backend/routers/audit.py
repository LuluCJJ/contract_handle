...
@router.post("/run")
async def run_audit(
    eflow_json: Annotated[UploadFile, File(description="E-Flow JSON 文件")],
    bank_doc: Annotated[UploadFile, File(description="银行申请表 (.doc/.docx/.pdf)")],
    id_documents: Annotated[List[UploadFile], File(description="证件图片列表 (支持多选上传)")],
):
    """
    执行完整的预审流程
    """
