...
2. 先讲一级用户旅程：配置环节、上传实施触发、最终结果和报告。
3. 在“配置环节”进入“模板配置中心”，展示模板资产、Template Plus 基准原文、区块配置和原文定位。
4. 演示配置 CRUD 原型：新增区块、复制区块、删除区块，并把区块绑定到模板原文行。
5. 进入“上传实施触发”，展示材料准备、上传材料、模板匹配与解析、路线 A、路线 B。
6. 用右上角“预审方案”一键切换方案 A/方案 B：方案 A 代表有 Template Plus，方案 B 代表无 Template Plus。
7. 进入“路线 A：模板 Plus”，展示申请书与 Template Plus 基准差异，以及差异分类。
8. 进入“路线 B：填写规范”，展示文档提取内容、参考值/规范、检查结论和证据。
9. 进入“最终结果和报告”，展示统一预审报告和多路径对比。
...
- 配置中心能展示区块规则配置原型。
- 顶部导航能按用户旅程聚类：配置环节、上传实施触发、最终结果和报告。
- 模板配置中心必须围绕模板版本、Template Plus 原文、锚点和检查规则，不再做宽泛配置中心。
- 模板配置中心能新增、复制、删除区块，并能点击区块定位到模板原文行。
