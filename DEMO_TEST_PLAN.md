...
5. 进入“路线 A：模板 Plus”，展示申请书与 Template Plus 基准差异，以及差异分类。
6. 进入“路线 B：填写规范”，展示文档提取内容、参考值/规范、检查结论和证据。
7. 进入“统一预审报告”，展示 10 个场景都可批量验证，其中包含 Word、PDF 和图片材料。
8. 进入“多路径对比”，比较路线 A、路线 B 和全文 Agent 的发现问题、模型调用和配置成本。
9. 选择 `PKG-CASE-003-ID-MISMATCH` 或 `PKG-CASE-004-NAME-MISMATCH`，展示证件号/姓名硬错误。
10. 选择 `PKG-CASE-005-ACTIVITY-RISK`，展示电子流与材料业务方向不一致。
11. 选择 `PKG-DEMO-001`，先用 Mock 展示稳定结果，再切真实 API 展示 Kimi 对新增高风险条款的解释。
12. 切到“上传材料”，粘贴或选择申请书文本，再粘贴模板基准，点击“生成材料包并进入审阅”，展示自定义材料也能进入同一流程。
